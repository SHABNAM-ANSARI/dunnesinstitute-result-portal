...
import { useEditableSubjects } from "@/hooks/useEditableSubjects";
import {
  GRADE_OPTIONS,
  PASSING_MARKS,
  MAX_MARKS,
...
const MarksheetEntry = ({ selectedClass, selectedTerm, userMobile }: MarksheetEntryProps) => {
  ...
  const { subjects, addSubject, updateSubject, deleteSubject, moveSubject, resetToDefault, isCustomized } =
    useEditableSubjects(selectedClass);
  ...
  const [newSubjectType, setNewSubjectType] = useState<"regular" | "credit">("regular");
  ...
  return (
    <div className="mt-10 report-card p-6 shadow-2xl rounded-xl print:shadow-none">
      ...
      {/* MANAGE SUBJECTS */}
      <div className="mb-4 border-2 border-accent/40 rounded-lg bg-accent/5 print:hidden">
        <button
          onClick={() => setShowSubjectMgr((v) => !v)}
          className="w-full flex items-center justify-between px-4 py-2 text-sm font-bold text-primary"
        >
          <span>
            ⚙️ Manage Subjects for {selectedClass}
            {isCustomized && <span className="ml-2 text-[10px] uppercase text-accent-foreground bg-accent px-2 py-0.5 rounded">Customized</span>}
          </span>
          <span>{showSubjectMgr ? "▲" : "▼"}</span>
        </button>
        {showSubjectMgr && (
          <div className="px-4 pb-4 space-y-3">
            <p className="text-[11px] text-muted-foreground">
              Edit names, change type (Regular = marks out of {MAX_MARKS}, Credit = grade only), or delete subjects. Changes are saved per class on this device.
            </p>
            <div className="overflow-x-auto border border-border rounded bg-background">
                  ...
                  {subjects.map((s, i) => (
                    <tr key={`${s.name}-${i}`} className="border-t border-border">
                      ...
                      <td className="px-2 py-1">
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            if (v && v !== s.name) {
                              const ok = updateSubject(i, { name: v });
                              if (!ok) {
                                toast.error("Name empty or duplicate");
                      ...
                      <td className="px-2 py-1">
                        <select
                          value={s.type}
                          onChange={(e) => updateSubject(i, { type: e.target.value as "regular" | "credit" })}
                          className="px-2 py-1 border border-border rounded bg-background text-xs"
                        >
                          <option value="regular">Regular (marks)</option>
                      ...
                      <td className="px-2 py-1 text-right">
                        <div className="inline-flex items-center gap-1">
                          <button
                            ...
                            title="Move up"
                            className="px-2 py-1 border border-border rounded bg-background text-xs hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            ↑
                          </button>
                          <button
                          ...
                          <button
                            onClick={() => {
                              if (confirm(`Delete subject "${s.name}"?`)) deleteSubject(i);
                            }}
                            className="ml-2 text-destructive font-bold text-xs hover:underline"
                          >
                            Delete
                          </button>
                        </div>
            ...
            <div className="flex flex-wrap items-end gap-2">
              <div className="flex flex-col">
                <label className="text-[10px] uppercase font-bold text-muted-foreground">New Subject</label>
                <input
              ...
              <div className="flex flex-col">
                ...
                <select
                  ...
                  onChange={(e) => setNewSubjectType(e.target.value as "regular" | "credit")}
                  className="px-2 py-1 border border-border rounded bg-background text-xs"
                >
                  <option value="regular">Regular (marks)</option>
                  <option value="credit">Credit (grade)</option>
              ...
              <button
                onClick={() => {
                  const ok = addSubject(newSubjectName, newSubjectType);
                  if (!ok) toast.error("Name empty or duplicate");
                  else {
                    toast.success(`Added ${newSubjectName}`);
                    setNewSubjectName("");
                  }
                }}
                className="px-3 py-1.5 bg-primary text-primary-foreground rounded font-bold text-xs"
              >
                ➕ Add
              </button>
              {isCustomized && (
                <button
                  onClick={() => {
                    if (confirm("Reset subjects for this class to defaults?")) resetToDefault();
                  }}
                  className="px-3 py-1.5 border-2 border-muted-foreground/30 rounded font-bold text-xs ml-auto"
                >
                  ↺ Reset to default
                </button>
              )}
