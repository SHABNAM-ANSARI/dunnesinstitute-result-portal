...
const Dashboard = ({ onLogout, userEmail, isAdmin, userMobile, onChangePassword }: DashboardProps) => {
  ...
  const { students: classStudents, loading: studentsLoading, isMock } = useStudentsByClass(selectedClass);
          ...
          <div className="bg-card p-8 rounded-xl shadow-md border border-primary/10">
            ...
            {selectedClass && (
              <div className="mt-4 p-3 bg-primary/5 rounded-lg text-sm">
                ...
                {isMock && !studentsLoading && (
                  <div className="mt-2 text-xs text-amber-700 bg-amber-100 border border-amber-300 rounded px-2 py-1">
                    ⚠️ Showing sample students — no records found in the database for this class. Import your class roster to replace this demo data.
                  </div>
                )}
              </div>
            )}
